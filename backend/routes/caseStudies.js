...
const fields = '*, author:profiles(full_name)'
...
router.get('/:id', authenticate, async (req, res) => {
  const column = req.query.by === 'slug' ? 'slug' : 'id'
  const { data, error } = await supabase.from('case_studies').select(fields).eq(column, req.params.id).single()
  if (error) return res.status(404).json({ error: 'Case study not found' })
...
router.post('/', authenticate, requireEditor, async (req, res) => {
  ...
  const { data, error } = await supabase.from('case_studies').insert(payload).select(fields).single()
  if (error) return res.status(400).json({ error: error.message })
...
router.patch('/:id', authenticate, requireEditor, async (req, res) => {
  if (req.body.status === 'published' && req.user.role !== 'admin') {
    return res.status(403).json({ error: 'Only admins can publish case studies' })
  }
  ...
  if (existingError) return res.status(404).json({ error: 'Case study not found' })
